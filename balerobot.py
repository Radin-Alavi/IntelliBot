...
async def get_tehran_air_quality():
    try:
        API_KEY = "a446a5cf-37d1-4beb-88ae-b32169503ab9"  
        COUNTRY = "Iran"
        STATE = "Tehran"
        CITY = "Tehran"
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status != 200:
                    return {
                        "success": False,
                        "error": f"خطای API با کد وضعیت {response.status}",
                        "24h_AQI": None,
                        "now_AQI": None
                    }
